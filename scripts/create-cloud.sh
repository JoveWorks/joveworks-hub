#!/usr/bin/env bash
# Create or update one cloud in a local JoveWorks Hub.
#
# Usage: scripts/create-cloud.sh <slug> <title>
set -euo pipefail
...
if command -v jq >/dev/null 2>&1; then
  ...
  echo "create-cloud.sh requires jq or python3 for safe encoding" >&2
  exit 1
fi
...
curl --silent --show-error --fail-with-body \
  ...
  --data-binary "$payload" \
  "${hub_url}/api/v1/clouds/${encoded_slug}"
